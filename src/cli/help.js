...
export function printHelp({ stdout = process.stdout } = {}) {
  const lines = [
    ...
    "  aim prime run codex [--codex <exact-label>]  # select Codex and start Prime with gpt-5.6-sol",
    "  aim prime run codex --codex <label> -- --no-env --offline --daemon-socket <abs> --session-dir <abs>",
    "  aim prime run claude  # select Claude and start Prime with claude-fable-5 in this directory",
    "  aim prime resume <path-or-id> [--rotate]  # human interactive convenience; no machine receipt",
    ...
    "  aim prime status [--brief-json] | aim prime uninstall [--provider <openai-codex|anthropic>]",
    "  aim prime create --request-json - | aim prime create status --request-json -",
    ...
    "  - Prime target mutations fail before effects when the persisted and resolved agent directories conflict.",
    "  - `aim prime create --request-json -` is prompt-free, journaled, idempotent, and preserves the other managed provider.",
    "  - Prime identity extension installation is explicit; use/run/resume/status/create never install it.",
    "  - Plain/rotating Prime resume remains a human interactive convenience, not a machine-control receipt lane.",
    ...
    "  --codex <auto|label|off>       Pi/Prime binding; `prime run codex` accepts only an exact label.",
    "  --claude <fable|opus|label|off> Pi/Prime Claude binding for new root sessions.",
    "  --replace-native-auth          Preserve one displaced native entry before explicit Pi/Prime replacement.",
    "  --brief-json                  Content-free typed view for `aim prime status`.",
    "  --request-json -              Strict stdin protocol for `aim prime create` and create status.",
