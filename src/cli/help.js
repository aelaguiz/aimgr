...
export function printHelp({ stdout = process.stdout } = {}) {
  const lines = [
    ...
    "  aim codex resume [<session-id>] # rotate, then resume the SAME thread with -p yolo (warns: the thread id continues under the new account, which links the accounts)",
    "  aim codex resume-fresh <session-id> | --last [--dry-run] [--no-goal] [--keep-reasoning] [--drop-compaction] [--max-copy-mb <n>] [--archive-source] [-- <codex args...>] # rotate account, copy the thread into a brand-new thread id, retire old identifiers, keep the memory, then resume the copy",
    ...
    "  aim routine run <id> [--manual] [--json]  # run a scheduled Prime, Codex, or Claude job from ~/.aimgr/config.yaml",
    ...
    "  aim mcp serve [--port <n>] [--bind <ip>] [--stdio]  # MCP server for this machine: aim_exec, aim_machine_info, aim_log_tail",
