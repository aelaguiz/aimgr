...
export function readHarnessTargetBriefStatus({
  targetId,
  targetState,
  authPath,
  pathConflict = false,
  ...
  try {
    if (fsImpl.existsSync(authPath)) {
      ...
      auth = parseAuthJson(fsImpl.readFileSync(authPath, "utf8"), authPath);
    }
  } catch {
    authUnreadable = true;
  }

  const projectProvider = (provider) => {
    const state = providerState(targetState, provider);
    const present = Boolean(auth) && Object.hasOwn(auth, provider);
    const malformed = present && !isObject(auth[provider]);
    const current = isObject(auth?.[provider]) ? auth[provider] : null;
    const managed = isAimHarnessExternalDescriptor(current);
    const hasOwnershipReceipt = isObject(state?.lastInstalledDescriptor);
    const receiptMatches = hasOwnershipReceipt
      && isDeepStrictEqual(current, state.lastInstalledDescriptor);
    const installed = present && Boolean(current);
    const owned = managed && (!hasOwnershipReceipt || receiptMatches);
    const conflict = authUnreadable || malformed || (hasOwnershipReceipt && !receiptMatches);
    let lastSelectedBinding = null;
    try {
      if (
        typeof state?.binding === "string"
        && normalizeLabel(state.binding) === state.binding
      ) {
        lastSelectedBinding = state.binding;
      }
    } catch {
      lastSelectedBinding = null;
    }
    return Object.freeze({
      managed,
      installed,
      owned,
      conflict,
      configuredBinding: managed ? current.binding : null,
      lastSelectedBinding,
    });
  };

  const providers = Object.freeze({
    codex: projectProvider(OPENAI_CODEX_PROVIDER),
    claude: projectProvider(ANTHROPIC_PROVIDER),
  });
  ...
  return Object.freeze({
    ...
    target: targetId,
    coordination,
    managed: values.some((entry) => entry.managed),
    installed: values.some((entry) => entry.installed),
    owned: values.some((entry) => entry.owned),
    conflict: Boolean(pathConflict) || authUnreadable || values.some((entry) => entry.conflict),
    pathConflict: Boolean(pathConflict),
    providers,
  });
}

export function readHarnessProviderProjectionSnapshot({
  targetState,
  authPath,
  provider,
  fsImpl = fs,
  ...
  const present = Object.hasOwn(auth, provider);
  const current = isObject(auth[provider]) ? auth[provider] : null;
  if (present && !current) throw new Error(`Refusing malformed current ${provider} auth.`);
  const state = providerState(targetState, provider);
  const lastInstalled = isObject(state?.lastInstalledDescriptor)
    ? state.lastInstalledDescriptor
    : null;
  const managed = isAimHarnessExternalDescriptor(current);
  return Object.freeze({
    present,
    managed,
    owned: Boolean(lastInstalled),
    ownershipConflict: Boolean(lastInstalled) && !isDeepStrictEqual(current, lastInstalled),
    replaceConflict: present && !managed,
    descriptor: managed ? structuredClone(current) : null,
    providerState: state ? structuredClone(state) : null,
  });
}

export async function restoreHarnessProviderProjection({
  targetState,
  authPath,
  provider,
  expectedCurrentDescriptor,
  priorDescriptor,
  priorProviderState,
  persistTargetState = async () => {},
  fsImpl = fs,
  ...
  assertProvider(provider);
  if (!isAimHarnessExternalDescriptor(expectedCurrentDescriptor)) {
    throw new Error("Projection rollback requires the exact current AIM descriptor.");
  }
  if (priorDescriptor !== null && !isAimHarnessExternalDescriptor(priorDescriptor)) {
    throw new Error("Projection rollback requires an AIM-owned or empty prior descriptor.");
  }
  const result = await withHarnessAuthTransaction(authPath, async (auth) => {
    const current = isObject(auth[provider]) ? auth[provider] : null;
    if (!isDeepStrictEqual(current, expectedCurrentDescriptor)) {
      throw new Error(`Refusing ${provider} projection rollback after target auth changed.`);
    }
    const next = { ...auth };
    if (priorDescriptor) next[provider] = structuredClone(priorDescriptor);
    else delete next[provider];
    return {
      result: { wrote: !isDeepStrictEqual(auth, next) },
      next: isDeepStrictEqual(auth, next) ? undefined : next,
    };
  }, { fsImpl, lockfileImpl });

  targetState.providers = isObject(targetState.providers) ? targetState.providers : {};
  if (priorProviderState) targetState.providers[provider] = structuredClone(priorProviderState);
  else delete targetState.providers[provider];
  if (Object.keys(targetState.providers).length === 0) delete targetState.providers;
  targetState.authPath = authPath;
  await persistTargetState();
  return Object.freeze({ wrote: Boolean(result?.wrote) });
}
