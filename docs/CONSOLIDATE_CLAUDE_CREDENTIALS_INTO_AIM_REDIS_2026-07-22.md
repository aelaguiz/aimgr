...
- Work:
  ...
  - [ ] P2.T4a (amended per A1/A2) — Project the canary from Redis into an AIM-owned per-label home on `Amir-M5`, verify exact identity through the allowlisted status boundary, and prove one official rotation publishes back to Redis advancing only that label — Keychain-free per A1 if the client supports file-only storage. Progress 2026-07-23: the local official `/usage` path, identity continuity, unchanged-token cleanup, and fresh-login v2→v3 publication all passed with no Keychain/global mutation; the provider did not issue a run-time token rotation, so no false rotation-advance claim is made.
  - [ ] P2.T4b (A4 gate OPEN) — Repeat projection/identity/rotation from `Amirs-M3-Max-2`, then clean up the frozen pre-A2 remote state. Progress 2026-07-23: `main` was deployed and the official remote `/usage` path passed twice from Redis v3 with zero model/API work, exact cleanup, and no real `security`/Keychain/global mutation. The provider again returned unchanged tokens, so the rotation-advance subcriterion remains empirically unobserved; the stale disposable file projection was retired, while the inert pre-A1 Keychain item remains untouched per A4.
- Verification:
