...
- Fixed in the fork (`main`, commit "resume must not fail on transient secondary daemon sockets"): the fan-out is best-effort once the default daemon answered, and discovery skips `prime-agent-forkserver-*` / `prime-agent-kernel-*` sockets. amir-server (`home`) runs from source via `~/workspace/prime-agent/prime-agent.sh` (tsx), so it got the same patch as branch `home/discovery-hotfix` on top of its older `f0c40b203` checkout; new clients pick it up immediately, no daemon restart needed. The M5 bundle install needs a rebuild + reinstall to carry it (§9).
- Idle eviction on home: the supervisor evicts a resumed worker with 0 clients after a few minutes when idle time (measured from the session's last turn, not the detach) is high and the fleet has ≥5 sessions (`Evicted idle worker … idleMinutes=251 sessions=5`). Do not rely on "resume, detach, let the user attach later"; either hand over the tmux (`tmux attach -t <name>`) or let the user resume fresh.
- SSH resets kill TUIs on home ("Read from remote host: Connection reset by peer"). Run Prime TUIs there inside `tmux` so the client survives the drop; the worker survives either way.
...
8. **Hotfix without stopping workers.** Reinstalling into the *same* prefix path and sending `node scripts/prime-daemon-wire.mjs restart` picks up the new supervisor code (the replacement is spawned from the same entrypoint path, which now holds new files) while every worker and session stays up. New TUIs launched afterwards get the new client code. Used tonight to add the path-resume fix under 16 live sessions.
   - 2026-09-04 addendum, worker-side fixes: the goal engine and the resume lookup run in **workers and TUIs**, not the supervisor. Reinstalling into the same prefix (`main-c8bc030`, the path the live supervisor spawns from) is enough for every *new* worker and TUI; no `restart` wire needed. Already-resident workers keep the old code until they cycle (eviction, crash, or a deliberate kill + resume). Verify with `grep -rl <new symbol> ~/.prime/installs/<prefix>/lib/node_modules/prime-agent/dist/bundle/`, not `prime-agent --version` (always 0.9.1).
