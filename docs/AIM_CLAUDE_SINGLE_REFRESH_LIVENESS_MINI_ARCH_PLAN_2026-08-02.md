...
| 2026-08-03 | Operator-authorized follow-up: an M3-published rotation may replace an unreceipted M5 cache only when Redis proves that exact local fingerprint was the rotation base. | Amir clarified the binding workflow: login and maintenance happen on `amirs-m3-max-new`; M5 consumes the shared Redis credential. Live `qa` evidence showed a safe but blocked transition: same stable account identity, local `stale_candidate`, Redis v32, no live fence or receipt, and v32 provenance naming the local fingerprint as its v31 rotation base. Decision: admit only that cryptographically linked successor case, then use the existing Redis projection/receipt path. Missing receipts without exact successor provenance, independent branches, identity conflicts, and newer local candidates still fail closed. The two pasted errors become account-first and remove the incorrect M5 `capture-native` advice. This supersedes only the prior row's assertion that the run lane was healthy; it adds no login path, state key, receipt schema, daemon, or shared-map change. |

# 10) Readiness Verdict

The original liveness implementation and rollout are complete. Its Scope and
Simplicity Contract held: no selection, lease, cache, receipt, pool-module,
provider-scoped run/capture/import/login, shared-map redesign, or CLI-surface
work entered that repair. The live incident acceptance matrix is recorded in
Phase 2 and the worklog. The later, operator-authorized M3-login/M5-consumer
correction is separately bounded and recorded in the 2026-08-03 Decision Log
entry above.
