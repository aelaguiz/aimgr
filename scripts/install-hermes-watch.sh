...
LABEL_MAC="com.funcountry.agents_host.aim_hermes_watch"
PLIST_MAC="/Library/LaunchDaemons/${LABEL_MAC}.plist"
LEGACY_PLIST_MAC_SYSTEM_AGENT="/Library/LaunchAgents/${LABEL_MAC}.plist"
SERVICE_LINUX="aim-hermes-watch.service"
TIMER_LINUX="aim-hermes-watch.timer"
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/install-hermes-watch.sh [options]
...
Options:
  ...
  --threshold-pct <n>         Rebalance when 5h remaining drops below this percent (default: 20)
...
Examples:
  bash scripts/install-hermes-watch.sh
  sudo bash scripts/install-hermes-watch.sh --user agents --interval-seconds 180
  npm run hermes-watch:install -- --print-only
  bash scripts/install-hermes-watch.sh --status
  sudo bash scripts/install-hermes-watch.sh --uninstall
EOF
}
...
render_linux_execstart() {
  if [[ -n "${HOST_ENV_WRAPPER}" ]]; then
    printf '%s\n' "/bin/bash ${HOST_ENV_WRAPPER} -- ${NODE_BIN} ${AIMGR_ENTRYPOINT} hermes watch --once --rotate-below-5h-remaining-pct ${THRESHOLD_PCT} --home ${TARGET_HOME}"
    return
  fi
  printf '%s\n' "${NODE_BIN} ${AIMGR_ENTRYPOINT} hermes watch --once --rotate-below-5h-remaining-pct ${THRESHOLD_PCT} --home ${TARGET_HOME}"
}
...
$(render_mac_program_arguments)
    ...
    <string>/tmp/agents_host_aim_hermes_watch.out.log</string>

    <key>StandardErrorPath</key>
    <string>/tmp/agents_host_aim_hermes_watch.err.log</string>
  </dict>
</plist>
EOF
}
...
render_linux_service() {
  cat <<EOF
[Unit]
Description=AIM Hermes watch one-shot
...
render_linux_timer() {
  cat <<EOF
[Unit]
Description=Run AIM Hermes watch every ${INTERVAL_SECONDS} seconds
...
install_watch() {
  ...
  if [[ "${OS_NAME}" == "Darwin" ]]; then
    ...
    echo "Logs: /tmp/agents_host_aim_hermes_watch.out.log /tmp/agents_host_aim_hermes_watch.err.log"
    return
  fi
