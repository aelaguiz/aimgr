#!/usr/bin/env bash
LABEL_MAC="${WATCH_LABEL_MAC}"
SERVICE_LINUX="${WATCH_SERVICE_LINUX}"
TIMER_LINUX="${WATCH_TIMER_LINUX}"
WATCH_INSTALL_ROOT="${AIMGR_WATCH_INSTALL_ROOT:-}"
WATCH_INSTALL_ROOT="${WATCH_INSTALL_ROOT%/}"

PLIST_MAC="${WATCH_INSTALL_ROOT}/Library/LaunchDaemons/${LABEL_MAC}.plist"
LEGACY_PLIST_MAC_SYSTEM_AGENT="${WATCH_INSTALL_ROOT}/Library/LaunchAgents/${LABEL_MAC}.plist"
SERVICE_LINUX_PATH="${WATCH_INSTALL_ROOT}/etc/systemd/system/${SERVICE_LINUX}"
TIMER_LINUX_PATH="${WATCH_INSTALL_ROOT}/etc/systemd/system/${TIMER_LINUX}"
...
usage() {
  cat <<EOF
Usage:
  bash scripts/${WATCH_SCRIPT_NAME} [options]
...
Examples:
  bash scripts/${WATCH_SCRIPT_NAME}
  sudo bash scripts/${WATCH_SCRIPT_NAME} --user agents --interval-seconds 180
  npm run ${WATCH_NPM_SCRIPT_PREFIX}:install -- --print-only
  bash scripts/${WATCH_SCRIPT_NAME} --status
  sudo bash scripts/${WATCH_SCRIPT_NAME} --uninstall
EOF
}
...
$(render_mac_program_arguments)
    ...
    <string>/tmp/${WATCH_LOG_STEM}.out.log</string>

    <key>StandardErrorPath</key>
    <string>/tmp/${WATCH_LOG_STEM}.err.log</string>
  </dict>
</plist>
EOF
}
...
render_linux_service() {
  cat <<EOF
[Unit]
Description=${WATCH_UNIT_DESCRIPTION}
...
render_linux_timer() {
  cat <<EOF
[Unit]
Description=${WATCH_TIMER_DESCRIPTION_PREFIX} every ${INTERVAL_SECONDS} seconds
...
install_watch() {
  ...
  if [[ "${OS_NAME}" == "Darwin" ]]; then
    ...
    echo "Logs: /tmp/${WATCH_LOG_STEM}.out.log /tmp/${WATCH_LOG_STEM}.err.log"
    return
  fi
