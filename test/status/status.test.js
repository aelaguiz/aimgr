...
test("plain status text contains only Codex accounts, Claude usage, and active labels", () => {
  ...
  assert.match(text, /pro7\s+max\/max_20x\s+usage_readable\s+yes\s+23%\s+3\.0h\s+45%\s+6\.0d\s+67%\s+6\.0d/);
  assert.match(text, /average\s+--\s+--\s+--\s+23%\s+3\.0h\s+45%\s+6\.0d\s+67%\s+6\.0d\s+--\s+--\s+all/);
  assert.match(
    text,
